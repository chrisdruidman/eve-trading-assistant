...
import type { SuggestionRun, SuggestedOrder } from '@eve-jita-ai/shared';
	...
	return (
		...
		>
			...
			<input
				...
				style={{ width: '14rem', padding: '.4rem .5rem' }}
			/>
			<button
				id="run-btn"
				type="submit"
			...
			<div id="status" style={{ minHeight: '1.25rem', color: '#444' }}>
				{status}
			</div>
		</form>
	);
	...
	return (
		...
		>
			...
			<select
				...
				onChange={(e) =>
					props.onChange({
						side: e.target.value as FiltersState['side'],
			...
			<input
				...
				min={0}
				placeholder="0"
				value={String(props.minMargin)}
				onChange={(e) =>
					props.onChange({ side: props.side, minMargin: Number(e.target.value || '0') })
				}
			/>
		</section>
	);
	...
	return (
		<div>
				...
				<tbody>
					{props.rows.map((s) => (
						<tr key={s.suggestion_id}>
							<td style={{ borderBottom: '1px solid #ddd', padding: '.5rem' }}>
								{s.type_id}
							</td>
							<td style={{ borderBottom: '1px solid #ddd', padding: '.5rem' }}>
								{s.side}
							...
							<td style={{ borderBottom: '1px solid #ddd', padding: '.5rem' }}>
								{formatCurrency(s.expected_margin)}
							</td>
							<td style={{ borderBottom: '1px solid #ddd', padding: '.5rem' }}>
								{s.rationale}
							</td>
			...
			<div
				...
				style={{ marginTop: '.75rem', display: 'flex', gap: '.5rem', alignItems: 'center' }}
			>
				<button onClick={props.onPrev} disabled={props.data.page <= 1}>
					Prev
				</button>
				<span className="page">{` Page ${props.data.page} `}</span>
				<button onClick={props.onNext} disabled={!props.data.hasMore}>
					Next
				</button>
			</div>
		</div>
